...
<script>
  ...
  let currentSpace
  let selectedStyle = { color: { conditions: [] } }
  let spaceUrl
  window.addEventListener("message", function (e) {
    ...
    if (e.data.type === "select") {
      ...
      const oldFeature = currentFeature
      ...
      showAllData(currentFeature, space)

    }

  });

  async function showAllData(feature, space) {
    showVoxelProperties(feature) //show voxel data
    showAdditionalProperties(space)

    const plateauPromise = plateauFetching = true
    ...
    const fudosanPromise = getFudosanData(space).then(d => {
      if (!d.length) return
      showData([["不動産情報", d]])
      updateIframeSize()
    }).finally(() => { fudosanFetching = false })
    return Promise.all([plateauPromise, fudosanPromise]).catch(e => console.error(e))
  }
  ...
  removeStyleBtn.addEventListener("click", function (e) {
    ...
    selectedStyle.color.conditions = []
    applyStyle()
  ...
  function buildStyleCondition() {
    const formColorCondition = getColorCondition()
    const selectedColorCondition = selectedStyle.color.conditions
    const defaultColorCondition = ["true", "color('#E8F1F2', 0.4)"]
    return [formColorCondition, selectedColorCondition, defaultColorCondition].filter(v => !!v)
  }
  ...
  function getColorCondition() {
    ...
    return ["\${" + attr + "}" + " " + operator + " " + value, "color('" + color + "', 0.5)"]
  }
...
reearth.on("select", (layerId) => {
  ...
  if (!layer || !reearth.layers.selectedFeature) return
  const featureId = reearth.layers.selectedFeature.properties?.id
  const oldFeatureId = currentFeatureId
  if (oldFeatureId === featureId) return
